...
func ApproveRegisterSideChain(id uint64, poly *poly_go_sdk.PolySdk, accArr []*poly_go_sdk.Account) {
	...
	for i, a := range accArr {
		...
		fmt.Printf("No%d: successful to approve: ( acc: %s, txhash: %s, chain-id: %d )", i, a.Address.ToBase58(), txhash.ToHexString(), id)
	}
	testcase.WaitPolyTx(txhash, poly)
}
...
func RegisterFiscoChain(poly *poly_go_sdk.PolySdk, acc *poly_go_sdk.Account) bool {
	...
	if err != nil {
		if strings.Contains(err.Error(), "already registered") {
			panic(fmt.Errorf("fisco chain %d already registered", config.DefConfig.FiscoChainID))
			return false
		}
		if strings.Contains(err.Error(), "already requested") {
			panic(fmt.Errorf("fisco chain %d already requested", config.DefConfig.FiscoChainID))
			return true
		}
		panic(fmt.Errorf("RegisterFiscoChain failed: %v", err))
	}
	testcase.WaitPolyTx(txhash, poly)
	fmt.Printf("successful to register fisco chain: ( txhash: %s )", txhash.ToHexString())
...
func RegisterChainsqlChain(poly *poly_go_sdk.PolySdk, acc *poly_go_sdk.Account) bool {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "already requested") {
			panic(fmt.Errorf("chainsql chain %d already requested", config.DefConfig.ChainsqlChainID))
			return true
		}
		panic(fmt.Errorf("RegisterFiscoChain failed: %v", err))
	}
